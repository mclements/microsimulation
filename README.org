...
The predict-function can also be used to calculate the various rates
described above. The predict-function also has the ability to predict
rates by a number of subgroups. The available subgroups are two
