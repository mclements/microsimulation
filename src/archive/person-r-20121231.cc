...
void Person::init() {
  if (R::runif(0.0,1.0)<0.2241) 
    scheduleAt(R::rweibull(exp(2.3525),64.0218),"Localised");
  scheduleAt(R::rexp(80.0),"Death");
}
...
extern "C" {
  ...
  void callPersonSimulation(int* inseed, double* parms, int *nin, double *out, int *nout) {
    ...
    //GetRNGstate(); // for non-user-defined uniform random number generators
    ...
    delete rng;
    //PutRNGstate(); // for non-user-defined uniform random number generators
  }
  ...
  void testRng(int* inseed, double *out) {
    ...
    rng->set();
    // output arguments to R
    out[0] = R::rnorm(0.0, 1.0);
    out[1] = R::rnorm(0.0, 1.0);
    // tidy up -- what needs to be deleted?
    delete rng;
    //PutRNGstate(); // for non-user-defined uniform random number generators
  }
