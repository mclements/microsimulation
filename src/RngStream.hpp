/**
 * @file    RngStream.hpp for multiple streams of Random Numbers
...
#ifndef RNGSTREAM_HPP
#define RNGSTREAM_HPP
 
#include <string>
#include <cstdlib> // implementation
#include <R.h>     // implementation: Rprintf and REprintf 

namespace ssim {

  inline double * nextSeed() { static double nextSeed_[6] = {12345,12345,12345,12345,12345,12345}; return nextSeed_; } // replaces static data member RngStream::nextSeed
...
//
inline double MultModM (double a, double s, double c, double m)
{
    double v;
    long a1;
...
// Works also when v = s.
//
inline void MatVecModM (const double A[3][3], const double s[3], double v[3],
                 double m)
{
    int i;
...
//
inline void MatMatModM (const double A[3][3], const double B[3][3],
...
//
inline void MatTwoPowModM (const double A[3][3], double B[3][3], double m, long e)
{
   int i, j;
...
//
inline void MatPowModM (const double A[3][3], double B[3][3], double m, long n)
{
    int i, j;
    double W[3][3];
...
// -1 otherwise.
//
inline int CheckSeed (const unsigned long seed[6])
{
    int i;
...
// constructor
//
inline RngStream::RngStream (const char *s) : name (s)
{
   anti = false;
   incPrec = false;
...
//
inline void RngStream::ResetStartStream ()
{
   for (int i = 0; i < 6; ++i)
      Cg[i] = Bg[i] = Ig[i];
}
...
//
inline void RngStream::ResetStartSubstream ()
{
   for (int i = 0; i < 6; ++i)
      Cg[i] = Bg[i];
}
...
//
inline void RngStream::ResetNextSubstream ()
{
...
//-------------------------------------------------------------------------
inline bool RngStream::SetPackageSeed (const unsigned long seed[6])
{
   if (CheckSeed (seed))
...
//-------------------------------------------------------------------------
inline bool RngStream::SetSeed (const unsigned long seed[6])
{
   if (CheckSeed (seed))
      return false;                   // FAILURE     
   for (int i = 0; i < 6; ++i)
...
//
inline void RngStream::AdvanceState (long e, long c)
{
...
//-------------------------------------------------------------------------
inline void RngStream::GetState (unsigned long seed[6]) const
{
   for (int i = 0; i < 6; ++i)
...
//-------------------------------------------------------------------------
inline void RngStream::IncreasedPrecis (bool incp)
{
   incPrec = incp;
}
...
//-------------------------------------------------------------------------
inline void RngStream::SetAntithetic (bool a)
{
   anti = a;
}
...
// Generate the next random number.
//
inline double RngStream::RandU01 ()
{
   if (incPrec)
      return U01d();
   else
...
//
inline int RngStream::RandInt (int low, int high)
{
