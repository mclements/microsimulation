...
namespace fhcrcTest {
  ...
  enum state_t {Healthy,Localised,Metastatic,ClinicalDiagnosis,ClinicalMetastaticDiagnosis,
		ScreenDiagnosis,ScreenMetastaticDiagnosis};
  
  enum event_t {toLocalised,toMetastatic,toClinicalDiagnosis,toClinicalMetastaticDiagnosis,toCancerDeath,toOtherDeath,toScreen, 
		toBiopsy,toScreenDiagnosis,toScreenMetastaticDiagnosis};
...
void FhcrcPerson::handleMessage(const cMessage* msg) {
  ...
  report.add(state,1,msg->kind,previousEventTime,now());
  ...
  case toMetastatic:
    ...
    scheduleAt(tmc+35.0,toClinicalMetastaticDiagnosis);
    break;
    
  case toClinicalDiagnosis:
    state = ClinicalDiagnosis;
  ...
  case toClinicalMetastaticDiagnosis:
    state = ClinicalMetastaticDiagnosis;
    remove_kind(toScreen);
    scheduleAt(now(), toBiopsy);
    if (R::runif(0.0,1.0) < 0.75)
  ...
  case toScreenMetastaticDiagnosis:
    state = ScreenMetastaticDiagnosis;
    remove_kind(toClinicalMetastaticDiagnosis); // competing events
    remove_kind(toScreen);
  ...
  case toBiopsy:
    ...
    case ClinicalMetastaticDiagnosis:
      break;
    case Localised:
      scheduleAt(now(), toScreenDiagnosis);
      break;
    case Metastatic:
      scheduleAt(now(), toScreenMetastaticDiagnosis);
      break;
    default:
